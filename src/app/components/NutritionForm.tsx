    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if(response.response && response.response.candidates){
          ...
          await saveNutritionToHistory(formattedData, session.user.id);
          ...
          console.warn(`Attempt ${attempt} failed: No candidates found in response.`);
          if (attempt < maxRetries) {
            await new Promise(resolve => setTimeout(resolve, retryDelay));
          } else {
            toast.error(`Não foi possível gerar a dieta após ${maxRetries} tentativas. Tente novamente mais tarde.`);
          }
        }
      } catch (error) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-6 border-b border-gray-100">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Gerar Dieta Personalizada</h2>
        <p className="text-gray-600">Preencha os dados abaixo para gerar sua dieta com IA</p>
      </div>
      
      <div className="p-6">
        <ApiStatus />
        
        <form onSubmit={createNutrition} className="space-y-6">
